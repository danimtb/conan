...
class LibB(ConanFile):
    ...
    def general_scope_options_test_package_test(self):
        client = TestClient()
        client.run("new example/1.0 -t")
        client.run("create . user/testing -o *:shared=True")
        # client.run("create . user/testing -o shared=True") This fails with test_package
        client = TestClient()
        client.run("new example/1.0")
        client.run("create . user/testing -o shared=True")  # But this works without test_package
